...
def calc_iwnll(model,iters,index_list ,all_add_latents,epoch,df_valid,ns=30):
    ...
    for inputs in tqdm(iters, desc="Evaluating PPL"):
        ...
        ce_tmp = []
        for _ in range(ns):
            log_gen, log_likelihood = model.iw_sample(input_ids, attention_mask,index_list,all_add_latents,epoch,inputs['texts'],df_valid)
