...
class GPT2VAEEncoder(GPT2PreTrainedModel):
    ...
    def __init__(self, config, wte, wpe, h, ln_f, dropout):
        ...
        self.wte = wte #token_embeddings
        self.wpe = wpe #position_embeddings
        self.drop = dropout #dropout layer
        self.h = h  #config.num_hidden_layers 这里用的是12
        self.ln_f = ln_f  #layer_norm层
        ...
        #这个是用来计算后验分布q的这个是因为x和c本来就表示在对应层里面了
        ...
        #self.prior_network用来计算先验的，这里需要区分是有无condition
        if self.is_cvae:
    ...
    def _recurrent_latent(self, post_latent, prior_latent_representaion, weight_ih, weight_hh): #对z的一个循环迭代，post代表=l，prior代表<l
    ...
    def get_prior(self, batch_size, device, condition=None, condition_mask=None):
        if condition is not None:
            condition_output = self(condition, attention_mask=condition_mask, compute_kl=False) #这里弄成false代表不逐层计算隐变量，只获取h_states
        ...
        prior_latent_representaion = torch.zeros(batch_size, self.latent_size).to(device) #<0
        prior_latent = torch.zeros_like(prior_latent_representaion) #=0
        all_prior_latent = ()
    ...
    def forward(
        ...
        prior_dist = Normal.get_standard(batch_size, self.latent_size, device)
        prior_latent_representaion = torch.zeros(batch_size, self.latent_size).to(device) #<0
        post_latent = torch.zeros_like(prior_latent_representaion) # =0
        all_post_latent = ()
        all_kl_loss = ()
        ...
        for i, (block, layer_past, pooling_layer, condition, recog_network, prior_network, weight_ih, weight_hh) in enumerate(loops):
            ...
            representation, _ = pooling_layer(hidden_states)  #平均注意力池化，这里是指对应层的
            all_hidden_states = all_hidden_states + (representation, )  #隐藏层相加
            if compute_kl and self.begin_layer <= i <= self.end_layer:
                # update prior latent representaion representaion代表<
                ...
                post_latent_representation = recog_network(torch.cat((representation, prior_latent_representaion), dim=-1)) #有x的后验
        ...
        return EncoderOutput(
            ...
            log_prior=log_prior,
            log_post=log_post,
            all_post_mu = all_post_mu,
            all_post_sigma = all_post_sigma
...
class GPT2VAEDecoderWithLMHead(GPT2PreTrainedModel):
    ...
    def __init__(self, config, wte, wpe, h, ln_f, dropout):
        ...
        # self.post_lambda_network = nn.ModuleList([nn.Linear(self.latent_size, 1) for _ in range(self.num_layers)])
    ...
    def prepare_inputs_for_generation(self, input_ids, past=None, **kwargs):
        ...
        add_latent = kwargs.get("add_latent", None)
    ...
    def forward(
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if self.begin_layer <= i <= self.end_layer:  #num_layer:bs:k  num_layer:bs:k:latent_size
                # print(all_latent[i - self.begin_layer].shape)
                # print(all_latent)
                # print(add_latent)
                latent = all_latent[i - self.begin_layer]*(1-args.retrieve_rate)+torch.sum(similarity[:,i - self.begin_layer].unsqueeze(-1)*add_latent[:,i - self.begin_layer]*args.retrieve_rate,dim=1)
                # print(all_latent[i - self.begin_layer].shape)
                # print(add_latent[i - self.begin_layer].shape)
                # stack_latent = torch.concat((all_latent[i - self.begin_layer].unsqueeze(1),add_latent[i - self.begin_layer]),dim=-2)
                # print(stack_latent.shape())
                # post_lambda = torch.softmax(self.post_lambda_network[i](stack_latent.view(-1,self.latent_size)).view(args.batch_size,11,1),dim=-2)#bs:k:1 
                # print(post_lambda)
                # latent = torch.sum(post_lambda*stack_latent,dim=1) #bs:k:latent_size
                # print(latent.shape)
            else:
                latent = None
            # Model parallel
...
class CRegaVAE(GPT2PreTrainedModel):
    def __init__(self, config,args):
        super().__init__(config)
        wte = nn.Embedding(config.vocab_size, config.hidden_size)           #wte weight_token_embedding缩写
        wpe = nn.Embedding(config.max_position_embeddings, config.hidden_size)  #wpe weight_position_embedding缩写

        h = nn.ModuleList([GPT2Block(config) for _ in range(config.num_hidden_layers)])  #gpt2模型的参数
        ln_f = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_epsilon)   
        drop = nn.Dropout(config.embd_pdrop)   #dropout p值

        self.encoder = GPT2VAEEncoder(config, wte, wpe, h, ln_f, drop) #构建encoder
        self.decoder = GPT2VAEDecoderWithLMHead(config, wte, wpe, h, ln_f, drop) #构建decoder
    ...
    def get_reduced_celoss(self, input_ids, attention_mask,add_latent,similarity):
        encoder_outputs = self.encoder(input_ids=input_ids, attention_mask=attention_mask)
        latent = encoder_outputs.latent
        decoder_outputs = self.decoder(input_ids, attention_mask=attention_mask, all_latent=latent,add_latent=add_latent,similarity=similarity)
    ...
    def get_celoss(self, input_ids, attention_mask, latent,add_latent,similarity):
    ...
    def get_neg_entropy(self, input_ids, attention_mask, ns=30): #采样过程，这里的latent才是隐变量，latentoutput是高斯分布的方差和均值
        log_post_list = []
        for _ in range(ns):
            encoder_outputs = self.encoder(input_ids=input_ids, attention_mask=attention_mask)
    ...
    def iw_sample(self, input_ids, attention_mask,index_list,all_add_latents,epoch,texts,df_valid):  
        encoder_outputs = self.encoder(input_ids=input_ids, attention_mask=attention_mask)
        latent = encoder_outputs.latent
        if epoch < self.args.bm25_epoch:
                temp_latents=[]
                temp_scores=[]
                for text in texts:
                    ...
                    temp_scores.append(result_score[:self.args.neighbors])#k bs:k
                    
                add_latent = torch.from_numpy(np.array(temp_latents)).to(self.args.device)
                similarity = torch.from_numpy(np.array(temp_scores,dtype =np.float32)).to(self.args.device)
                add_latent = add_latent.permute(2,0,1,3)
                similarity = similarity.unsqueeze(0)
                similarity = similarity.expand(self.args.num_layers,similarity.shape[1],similarity.shape[2])
                similarity = torch.softmax(similarity,dim=-1)
        else:
            with torch.no_grad():
                query_output = self.encoder(input_ids=input_ids, attention_mask=attention_mask)
            add_latent=[]
            similarity=[]
                # print(query_output.latent)
            tmp_latent = np.stack([i.cpu().numpy() for i in query_output.latent],axis=0)
            for v in range(12):
                query_vector=tmp_latent[v]
                distances, indices = index_list[v].search(query_vector, k = self.args.neighbors) #bs:k bs:k
                similarity.append(distances)#nl:bs:k
                temp_bs_latent=[]
                for g in range(len(indices)):
                    temp_latent = []
                    for k in range(self.args.neighbors):
            ...
            add_latent = torch.from_numpy(np.array(add_latent)).to(self.args.device)
            similarity = torch.from_numpy(np.array(similarity)).to(self.args.device)
            similarity = torch.softmax(similarity,dim=-1)
            
        log_prior = encoder_outputs.log_prior
        log_post = encoder_outputs.log_post
        log_gen = -self.get_celoss(input_ids, attention_mask, latent,add_latent,similarity)
    ...
    def forward(self, input_ids, labels=None, attention_mask=None, condition=None, condition_mask=None,add_latent=None,similarity=None):
        if condition is not None:
        ...
        decoder_output = self.decoder(input_ids, args = self.args,attention_mask=attention_mask, all_latent=latent,add_latent=add_latent,similarity=similarity)
