...
class VAEDataset(Dataset):
    ...
    @staticmethod
    def create_mask(num_tokens, max_len):  #创建attention_mask
    ...
    def collate_fn(self, texts): #加上这两个id,使用动态padding
        
        samples = [[self.tokenizer.bos_id] + self.tokenizer.encode(s) + [self.tokenizer.eos_id] for s in texts]
        ...
        attention_mask = self.create_mask(torch.LongTensor(length_list), max_t) #LongTensor
        return {
...
class WPDataset(Dataset): 
    def __init__(self, source_path,target_path, tokenizer, device=torch.device('cuda:0'), max_length=700, add_prefix=False, add_special_token=False):
        self.source = []
        self.target = []
        self.tokenizer = tokenizer
        self.device = device
        ...
        for i in tqdm.tqdm(range(len(targets)), desc='Loading data...'):
            line_s = sources[i].strip()
            source = line_s.replace('<newline>', '\n')
            
            line_t = targets[i].strip()
            target = line_t.replace('<newline>', '\n')
    ...
    def collate_fn(self, samples):
        ...
        return {
            'input_ids': new_target.cuda(),
            'attention_mask': target_attention_mask.byte().cuda(),
            'labels': new_labels.cuda(),
            'condition': new_source.cuda(),
            'condition_mask': source_attention_mask.byte().cuda(),
        }