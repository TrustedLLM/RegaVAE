...
class GPT2VAEDecoderWithLMHead(GPT2PreTrainedModel):
    ...
    def __init__(self, config, wte, wpe, h, ln_f, dropout,args):
        super().__init__(config)
    ...
    def prepare_inputs_for_generation(self, input_ids, past=None,latent=None, **kwargs):
    ...
    def forward(
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if self.begin_layer <= i <= self.end_layer:  
                latent = all_latent[i - self.begin_layer]*(1-self.args.retrieve_rate)+torch.sum(similarity[i - self.begin_layer].unsqueeze(-1)*add_latent[i - self.begin_layer]*self.args.retrieve_rate,dim=1)
...
class RegaVAE(GPT2PreTrainedModel):
    def __init__(self, config,args):
        ...
        self.decoder = GPT2VAEDecoderWithLMHead(config, wte, wpe, h, ln_f, drop,args)
    ...
    def get_celoss(self, input_ids, attention_mask, latent,add_latent,similarity):
        ...
        decoder_outputs = self.decoder(input_ids, args = self.args,attention_mask=attention_mask, all_latent=latent,add_latent=add_latent,similarity=similarity)
        lm_logits = decoder_outputs.logits
    ...
    def iw_sample(self, input_ids, attention_mask,index_list,all_add_latents,epoch,texts,df_valid):  #采样过程
        encoder_outputs = self.encoder(input_ids=input_ids, attention_mask=attention_mask)
        latent = encoder_outputs.latent
        if epoch < self.args.bm25_epoch:
                ...
                for text in texts:
                    ...
                    temp_latents.append([all_add_latents[item] for item in result_index[:self.args.neighbors]]) #bs,k,num_layer,latent_size
